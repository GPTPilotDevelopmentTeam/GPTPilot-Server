...
def _monitor_microphone():
    ...
    _stt_log("Listening to microphone (continuous mode)...")
    ...
    with stream:
        ...
        while _is_running:
            ...
            if silent_chunks * (BLOCK_SIZE / SAMPLE_RATE) > SLIENCE_DURATION and has_speak:
                ...
                _stt_log(f"Detected speech with {len(audio) / SAMPLE_RATE:.2f} seconds.")
            ...
            if len(recording) * (BLOCK_SIZE / SAMPLE_RATE) > MAX_RECORDING_DURATION:
                if has_speak:
                    audio = np.concatenate(recording)
                    _stt_log("Max duration reached, auto cutting...")
                    push_transcribe_audio(audio)
                else:
                    _stt_log("Record nothing, skipping.")
                recording = []
                silent_chunks = 0
                has_speak = False
...
def _transcribing_audio():
    ...
    while _is_running:
        ...
        _stt_log(f"Transcribing audio with {len(audio) / SAMPLE_RATE:.2f} seconds...")
...
def start():
    ...
    _stt_log("Starting audio transcribe system...", True)
    ...
    _stt_log("Done! You may start talking.", True)
    _stt_log("Audio transcribe system started with the following parameters:")
    _stt_log(f"THRESHOLD: {THRESHOLD}")
    _stt_log(f"SLIENCE_DURATION: {SLIENCE_DURATION}")
    _stt_log(f"SAMPLE_RATE: {SAMPLE_RATE}")
    _stt_log(f"BLOCK_SIZE: {BLOCK_SIZE}")
    _stt_log(f"MAX_RECORDING_DURATION: {MAX_RECORDING_DURATION}")


def stop():
    """Stop the audio processing functionality."""
    _stt_log("Stopping audio transcribe system...", True)
    ...
    _stt_log("Audio transcribe system stopped.", True)
